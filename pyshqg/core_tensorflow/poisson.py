...
class QGPoissonSolver:
    ...
    spec_qp : tf.Tensor, shape (Nlevel, 2, T+1, T+1)
        The planet vorticity, corrected by the orography, in spectral space $\hat{q}^{\mathsf{p}}$.
    q_to_psi_coupling : tf.Tensor, shape (Nlevel, Nlevel, T+1)
    ...
    def precompute_q_to_psi_coupling(
        ...
        # convert to tf.Tensor
        return tf.convert_to_tensor(q_to_psi_coupling)
    ...
    def q_to_total_q(self, spec_q):
        ...
        Parameters
        ----------
        spec_q : tf.Tensor, shape (..., Nlevel, 2, T+1, T+1)
